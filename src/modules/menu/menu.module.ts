...
import {
  ...
  deleteMenuHandler,
  fetchMenuHandler,
} from '@api/modules/menu/adapters/http/in';
...
export class MenuModule {
  static register(app: Express) {
    ...
    const menuCreationPipeline = [
      asyncHandler(createMenuValidation),
      asyncHandler(createMenuHandler(new CreateMenuUseCase(repository))),
    ];
    const menuDeletionPipeline = [
      validateIdParam,
      asyncHandler(deleteMenuHandler(new DeleteMenuUseCase(repository))),
    ];
